                ...
                <div class="card-body">
                    ...
                    <div class="row mt-2 mb-2">
                        <div class="col-sm-12 col-lg-6"> <!--ชนิดครุภัณฑ์-->
                            <div class="form-group">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type_id">
                                    <option value="" hidden selected></option>
                                    @foreach($peripheraltypes as $peripheraltype)
                                        <option value="{{ $peripheraltype['id'] }}"{{ old('type_id',$peripheral->type_id) == $peripheraltype['id'] ? 'selected' : ''}}>{{ $peripheraltype['name'] }}</option>
                                    @endforeach
                                </select>
                                @error('type_id')
                    ...
                    <div class="row mt-2 mb-2">
                        ...
                        <div class="col-sm-12 col-lg-6"><!--ห้อง-->
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('location_id') is-invalid @enderror" name="room" placeholder="กรุณาระบุห้อง" id="room_autocomplete" value="{{ old('room',$peripheral->PeripheralRoom->name) }}"/>
                                @error('location_id')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="form-group"> 
                                ...
                                <select class="form-select @error('section_id') is-invalid @enderror" name="section_id" id="section">
                                    ...
                                    @foreach($sections as $section)
                                        <option value="{{ $section['id'] }}" {{ old('section_id',$peripheral->section_id) == $section['id'] ? 'selected' : '' }}>{{ $section['name'] }}</option>
                                    @endforeach
                                </select>
                                @error('section_id')
                                    <div class="invalid-feedback">
                    ...
                    <div class="row mt-2 mb-2">
                        <div class="col-sm-12 col-lg-3">
                            <label for="multi_user" class="form-label">จำนวนผู้ใช้งาน</label><br>
                            <div class="form-check-inline">
                                @foreach ($multiusers as $multiuser)
                                    ...
                                    <label class="radio">{{ $multiuser['name'] }}</label>
                                @endforeach
                                @error('multi_user_id')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                        ...
                        <div class="col-sm-12 col-lg-6"> <!--ตำแหน่งผู้ใช้งาน-->
                            <div class="form-group">
                                ...
                                <select class="form-select @error('position_id') is-invalid @enderror" name="position_id" id="position" class="form-control">
                                    <option value="" hidden></option>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check">
                                    @foreach ($owners as $owner)
                                        <input class="form-check-input @error('owner_id') is-invalid @enderror" type="radio" name="owner_id" id="owner" value="{{ $owner['id'] }}" {{ old('owner_id',$peripheral->owner_id) == $owner['id'] ? 'checked' : '' }}>
                    ...
                    <div class="row mt-2 mb-2">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select @error('asset_status_id') is-invalid @enderror" name="asset_status_id" id="asset_status">
                                    ...
                                    @foreach($asset_statuses as $asset_status)
                                        <option value="{{ $asset_status['id'] }}" {{ old('asset_status_id',$peripheral->asset_status_id) == $asset_status['id'] ? 'selected' : ''}}>{{ $asset_status['name'] }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-sm-12 col-lg-6"> <!--สถานะการใช้งาน-->
                            <div class="form-group">
                                ...
                                <select class="form-select @error('asset_use_status') is-invalid @enderror" name="asset_use_status_id" id="asset_use_status">
                                    <option value="" hidden selected></option>
                                    @foreach($asset_use_statuses as $asset_use_status)
                                        <option value="{{ $asset_use_status['id'] }}" {{ old('asset_use_status_id',$peripheral->asset_use_status_id) == $asset_use_status['id'] ? 'selected' : ''}}>{{ $asset_use_status['name'] }}</option>
                                    @endforeach
                                </select>
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-2 mb-2">
                        ...
                        <div class="col-sm-12 col-lg-6"> <!--วัสดุสึกหรอสิ้นเปลือง-->
                            <div class="form-group">
                                <label for="supply_consumables" class="form-label">สามารถเบิกวัสดุสึกหรอสิ้นเปลืองได้</label>
                                <div class="form-check">
                                    @foreach($supplies as $supply)
                                        <input class="form-check-input @error('supply_consumables_id') is-invalid @enderror" type="radio" name="supply_consumables_id" id="supply_consumables" value="{{ $supply['id'] }}" {{ old('supply_consumables_id',$peripheral->supply_consumables_id) == $supply['id'] ? 'checked' : '' }}>
                                        <label class="form-check-label" for="supply_consumables">{{ $supply['name'] }}</label><br>
                                    @endforeach
                                    @error('supply_consumables_id')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                    ...
                    <div class="row mt-2 mb-2">
                        <div class="col-sm-12 col-lg-6"> <!--วิธีการเชื่อมต่อ-->
                            <div class="form-group">
                                <label for="asset_use_status" class="form-label">วิธีการเชื่อมต่อ</label>
                                <select class="form-select @error('connectivity_id') is-invalid @enderror" name="connectivity_id" id="connectivity">
                                    <option value="" hidden selected></option>
                                    @foreach ($peripheralconnections as $peripheralconnection)
                                        <option value="{{ $peripheralconnection['id'] }}"{{ old('connectivity_id',$peripheral->connectivity_id) == $peripheralconnection['id'] ? 'selected' : ''}}>{{ $peripheralconnection['name'] }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-sm-12 col-lg-6"> <!--ip address-->
                            <div class="form-group">
                                <label for="ip_address" class="form-label">IP address ของเครื่อง</label>
                    ...
                    <div class="row mt-2 mb-2">
                        <div class="col-sm-12 col-lg-6"> <!--Lan outlet-->
                            <div class="form-group">
                                <label for="share_name" class="form-label">Lan Outlet No.</label>
                        ...
                        <div class="col-sm-12 col-lg-6"> <!--share-->
                            <div class="form-group">
                                <label for="is_shared" class="form-label">สถานะการใช้งานร่วมกัน</label><br>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-lg-6"><!--วิธี share-->
                            <div class="form-group">
                                <label for="share_method" class="form-label">วิธีการ share</label>
                                <div class="form-check">
