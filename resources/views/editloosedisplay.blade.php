...
@section('content')
    <div class="container-fluid">
        <div class="col-12 mx-auto">
            <form action="{{url('/loosedisplay/update',$loosedisplay->id)}}" method="post">
                @csrf
                <div class="card mt-4">
                    ...
                    <div class="card-body">    
                        <div class="row mb-2 mt-2">
                            ...
                            <div class="col-sm-12 col-lg-6"> <!--รหัสครุภัณฑ์-->
                                <div class="form-group">
                                    <label for="display_pid" class="form-label">รหัสครุภัณฑ์</label> 
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-check-inline pl-2">
                                        @foreach ($owners as $owner)
                                            <input type="radio" class="form-check-input ml-1 @error('owner_id') is-invalid @enderror" name="owner_id" id="owner1" value="{{ $owner['id'] }}" {{ old('owner_id',$loosedisplay->owner_id) == $owner['id'] ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('section_id') is-invalid @enderror" name="section_id" id="section">
                                        ...
                                        @foreach ($sections as $section)
                                            <option value="{{$section['id']}}" {{old('section_id',$loosedisplay->section_id ) == $section['id'] ? 'selected': ''}}>{{$section['name']}}</option>
                                        @endforeach
                                    </select>
                                    @error('section_id')
                                        <div class="invalid-feedback">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('position_id') is-invalid @enderror" name="position_id" id="position">
                                        ...
                                        @foreach ($positions as $position)
                                            <option value="{{$position['id']}}" {{old('position_id',$loosedisplay->position_id) == $position['id'] ? 'selected' : ''}}>{{$position['name']}}</option>
                                        @endforeach
                                    </select>
                                    @error('position_id')
                                        <div class="invalid-feedback">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="asset_status_id" id="asset_status" class="form-select @error('asset_status_id') is-invalid @enderror">
                                        ...
                                        @foreach ($asset_statuses as $asset_status)
                                            <option value="{{$asset_status['id']}}" {{old('asset_status_id',$loosedisplay->asset_status_id) == $asset_status['id'] ? 'selected' : ''}}>{{$asset_status['name']}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="asset_use_status_id" id="asset_use_status" class="form-select @error('asset_use_status_id') is-invalid @enderror">
                                        ...
                                        @foreach ($asset_use_statuses as $asset_use_status)
                                            <option value="{{$asset_use_status['id']}}" {{old('asset_use_status_id',$loosedisplay->asset_use_status_id) == $asset_use_status['id'] ? 'selected' : ''}}>{{$asset_use_status['name']}}</option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-2 mt-2">
                            <div class="col-sm-12 col-lg-6">
                                <div class="form-group">
                                    <label for="display_ratio" class="form-label">สัดส่วนจอภาพ</label> <!--สัดส่วนจอภาพ-->
                                    <select name="display_ratio_id" id="display_ratio" class="form-select @error('display_ratio_id') is-invalid @enderror">
                                        ...
                                        @foreach ($displayratios as $displayratio)
                                            <option value="{{$displayratio['id']}}" {{old('display_ratio_id',$loosedisplay->display_ratio_id) == $displayratio['id'] ? 'selected' : ''}}>{{$displayratio['name']}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-12 col-lg-6"> <!--สัญญาณที่รองรับ-->
                                <div class="form-group">
                                    ...
                                    <div class="form-check">
                                        <label class="checkbox-inline"><input type="checkbox" name="is_vga" value="1" {{old('is_vga',$loosedisplay->is_vga) == 1 ? 'checked' : ''}}> VGA</label>
                                    </div>
                                    <div class="form-check">
                                        <label class="checkbox-inline"><input type="checkbox" name="is_dvi" value="1" {{old('is_dvi',$loosedisplay->is_dvi) == 1 ? 'checked' : ''}}> DVI</label>
                                    </div>
                                    <div class="form-check">
                                        <label class="checkbox-inline"><input type="checkbox" name="is_hdmi" value="1" {{old('is_hdmi',$loosedisplay->is_hdmi) == 1 ? 'checked' : ''}}> HDMI</label>
                                    </div>
                                    <div class="form-check">
                                        <label class="checkbox-inline"><input type="checkbox" name="is_dp" value="1" {{old('is_dp',$loosedisplay->is_dp) == 1 ? 'checked' : ''}}> DisplayPort</label>
                                    </div>
                                </div>
                            </div>
                        </div>
